...
- **Documentation:** https://github.com/yenmangu/ci-ms-3-studystack/
...
| Subject AND filter raised AttributeError                   | `ModelMultipleChoiceFilter` passed a list of model instances to the filter method, not a QuerySet; code incorrectly assumed `.values_list()` was available                 | Treated selected values as an iterable and extracted subject IDs via list comprehension before filtering                                                   | Commit ID: [5c61383](https://github.com/yenmangu/ci-ms-3-studystack/commit/5c6138358d72130f82641418e4d1618ed3f07fcb) |
| Resource form displayed incorrect heading and button label | Shared form template always rendered “Add Resource” regardless of context, causing a mismatch between page intent (create vs edit) and UI messaging                        | Passed an explicit `heading` context variable from create and update views and used it consistently for the form heading and submit button                 | Commit ID: [ce67d09](https://github.com/yenmangu/ci-ms-3-studystack/commit/ce67d0918ba71a8ccce1053603bb298342475877) |
| Signup displayed incorrect “Welcome back” message          | django-allauth automatically logs users in after registration, triggering the existing login signal handler and displaying a misleading welcome-back message for new users | Added a dedicated signup signal handler and suppressed the login message when the login originated from signup, ensuring context-appropriate user feedback | Commit ID: [4fd35a3](https://github.com/yenmangu/ci-ms-3-studystack/commit/4fd35a3ccc4032bde3f265d0dfe25dc54b439226) |
